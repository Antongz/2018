...
uint64_t memory::read_doubleword (uint64_t address) {
    ...
    uint64_t temp;
    uint64_t block_num = (uint64_t)(address/(BLOCK_SIZE*8));
    ...
    if(it==Blocks.end()){
        Blocks.insert(pair<uint64_t,uint64_t*>(block_num,new uint64_t[BLOCK_SIZE]{0}));
    }
        ...
        while (true) {
            ...
            case 0x00:  // Data record
                for (unsigned int i = 0; i < record_length; i++) {
                    ...
                    write_byte(load_address,record_data,0x00000000000000ff );
                    //write_doubleword(load_address,record_data,0x00000000000000ff);
                    byte_count++;
                }
                break;
...
void memory::write_byte(uint64_t address, uint64_t data, uint64_t mask) {
  ...
  //cout << "old data: " << hex << old_data << " new_word: " << hex << new_word << endl;
  Blocks[block_num][offset_] = new_word;

   //Blocks[block_num][offset] = (new_word & 0x00000000000000ff);
