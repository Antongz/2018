 ...
 *
 *        Version:  1.0
 *        Created:  04/12/2018 01:27:44 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
uint64_t test = 0x00;
bool checkVist(uint64_t pc){
    if(pc % 0x0000000000000008==0)
        return true;
    return false;
}

int check_OffBits(uint64_t p){
    int steps = 0;
    while(p%0x0000000000000008!=0){
        p--;
        steps++;
    }
    //return (int)p%0x0000000000000008;
    return steps;
}
...
uint64_t getImm_I(uint64_t ir){
    uint64_t temp = (ir >> 20) & 0xfff;
    //uint64_t temp = ((ir & 0x00000000ffe00000)>>20);
    return temp;
}
...
// ADDI adds a sign-extended 12-bits immediate to register rs1. Overflow is ignored
// and the result is the low XLEN bits of the result
void EXE_ADDI(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    if((imm & 0x0000000000000800) == 0x0000000000000800){
        imm = ~imm;
        imm += 1;
        //cout << "this is imm " << imm << "  ~imm: " << ~imm<< endl;
        //cout << "process.get_reg: " << processor->get_reg(rs1) << endl;
...
void EXE_JALR(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    /*
    //X[rd] = PC +4;
    processor->set_reg(rd,processor->get_PC() + 4);
    uint64_t imm = getImm_I(ir);
    if((imm & 0x00000800) == 0x00000800){
        ...
        //PC = X[getRS1(ir)] - imm;
        processor->set_pc(X[getRS1(ir)] - imm -4);
    }else {
        if((imm % 2) != 0){
            imm -= 1;
        }
    ...
    if(verbose){
        cout<< dec  <<"jalr: rd = " <<rd << ", rs1 = " << rs1 << ", immed_I = " << setw(16)<<setfill('0') << hex <<imm<<endl;
    }
...
//the 32bit result is sign-extended to 64 bits
void EXE_LUI(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    if(((imm & 0x0000000080000000 )== 0x0000000080000000)){
        ...
        ret = (imm & 0x00000000ffffffff);
    }
    if(verbose){
        cout<< dec  <<"lui: rd = " <<rd << ", immed_U = "  << hex <<ret <<endl;
    }
    //X[rd] = imm;
...
void EXE_JAL(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    int rd = getRD(ir);
    if(verbose){
        cout << " before change immed_J = " << setw(16)<<setfill('0') << hex <<imm<<endl;
    }
...
// lw loads 32-bit value from memory and sign-extends this to 64bits
...
void EXE_SB(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    if(verbose){
        cout<< dec  <<"sb: rs1 = "<< rs1 << ", rs2 = "<< rs2 << ", immed_S = " << setw(16)<<setfill('0') << hex <<imm<<endl;
    }
    ...
    if(verbose){
        cout << "X[rs2]: " << hex << X[rs2] << endl;
        cout << "offbits " << offbits << endl;
        cout << "Memory write word: address = " << hex << setw(16) << setfill('0') << ret  << ", data = "
             << setw(16) << setfill('0') << hex << write_data << endl;
        cout << "mask: " << hex << setfill('0') << mask << endl;
    }
    /*test here*/
    mem->write_doubleword(data_address,write_data,mask);

    if(verbose){
        cout << "show the address in ret: ";
        mem->show_address(data_address);
        cout << "show rs2: ";
        processor->show_reg(rs2);
    }
}
...
void EXE_SH(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    if(processor->get_reg(rs1)%2==0){
        ...
        if(verbose){
            cout<< dec  <<"sh: rs1 = "<< rs1 << ", rs2 = "<< rs2 << ", immed_S = " << setw(16)<<setfill('0') << hex <<imm<<endl;
        }
        uint64_t ret = imm;
        ret = imm + X[rs1];
    ...
    //processor->count_cycle(ret % 4==3 ? 4 : 2);
}
...
void EXE_SRAI(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    uint64_t ret = X[rs1];
    //cout << "Before:   srai: " << hex << ret << endl;
    uint64_t bs = imm % 64;
    uint64_t s = ret & 0x8000000000000000;

    //cout << "bs: " << dec << bs << "   , s: " << s << endl;;
    for(int i=0;i<bs;i++){
        ret >>= 1;
        if(s){
...
void EXE_SRA(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    uint64_t bs = X[rs2] % 64;
    uint64_t s = ret & 0x8000000000000000;

    for(uint64_t i=0;i<bs;i++){
        ret >>= 1;
        if(s) ret |= 0x8000000000000000;
    }
...
void EXE_ADDIW(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    if(verbose){
        cout << dec <<"addiw: rd = " <<rd<< ", rs1 = " << rs1 << ", immed_I = " << hex <<imm<<endl;
    }
    ...
    //check upper 32bits signed extend
...
void EXE_SLLW(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    uint64_t rs1 = getRS1(ir);
    uint64_t rs2 = getRS2(ir);
    uint64_t left = (X[rs2] & 0x000000000000001f);
    uint64_t ret = (X[rs1] & 0x00000000ffffffff);
    uint64_t s = ret & 0x0000000080000000;

    // cout << "left:" << left << "   ret: " << hex << ret << "  s: " << s << endl;
    ...
    X[rd] = ret;
    if(verbose){
        cout<< dec  <<"sllw: rd = " <<rd << ", rs1 = "<< rs1 << ", rs2 = " << rs2  <<endl;
    }
    processor->count_cycle(1);
}
void EXE_SRLW(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    uint64_t rs1 = getRS1(ir);
    uint64_t rs2 = getRS2(ir);
    uint64_t right = (X[rs2] & 0x000000000000001f);
...
void EXE_SRAW(memory* mem,processor* processor,uint64_t ir, uint64_t* X,uint64_t& PC,bool verbose){
    ...
    uint64_t bs = (X[rs2] & 0x000000000000001f);
    int s = ret & 0x0000000080000000;

    for(uint64_t i=0;i<bs;i++){
        ret >>= 1;
        if(s) ret |= 0x0000000080000000;
    }
...
// loads 64-bit value from memory into register
...
void checkInstruction(memory* memory,processor* processor,uint64_t* X,uint64_t& PC,uint64_t ir,bool verbose){
    ...
    if(processor->mstatus_reg!=0x0000000200000000 ||
        ...
        if(1){
            ...
            if(getOPCODE(ir) == 0x0000000000000013 && getFUNC3(ir) == 0x0000000000000000){
            ...
            }else if((getOPCODE(ir) == 0x13 && getFUNC3(ir) == 0x01 &&
                      (getFUNC7(ir) == 0x00 || getFUN7_64(ir) == 0x00))){
                ...
                cout << "Illegal instruction: " << setw(8) << hex << setfill('0') << ir  <<  endl;
                //cout << "getfun7: " << hex <<  getFUNC7(ir) << "   ;  getfun764: " << hex << getFUN7_64(ir) << endl;
            }
        }else{
