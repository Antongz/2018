...
static void ybn_srli(ybn_t result, ybn_t a, uint16_t shiftBit, int shift) {
    int i = 0;
    uint32_t carry = 0x00;
    uint32_t tempData = 0x00;

    for (i=a->ybn_len - 1; i>=0; i--) {
        tempData = (uint32_t) ((carry) << 16);
        tempData = (uint32_t) (tempData | a->ybn_data[i]);
        tempData >>= shift;

        carry = (uint32_t) (a->ybn_data[i] & shiftBit);
        result->ybn_data[i] = (uint16_t) tempData;
    }

    result->ybn_len = ybn_reallen(result);
}
