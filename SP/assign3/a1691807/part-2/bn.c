...
//        base := (base * base) mod modulus
        if ((exp->bn_data[0] & 0x0001) == 0x0001) {
            bn_t iterator = bn_alloc();
            if (iterator == NULL) return -1;
            if (bn_mul(iterator, tempResult, base) == -1) return -1;
            if (copyBN(tempResult, iterator) == -1) return -1;
        ...
        bn_srli(exp, exp, 0x01, 1);
        {
            bn_t iterator = bn_alloc();
            if (iterator == NULL) return -1;
            if (bn_mul(iterator, base, base) == -1) return -1;
            if (copyBN(base, iterator) == -1) return -1;
            if (copyBN(tempBase, base) == -1) return -1;
            if (bn_div(temp, base, tempBase, modulus) == -1) return -1;
            bn_free(iterator);
        }
...
int bn_sqr(bn_t result, bn_t a) {
    if (bn_mul(result, a, a) == -1) return -1;
    return 0;
}
