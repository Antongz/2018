...
ybn_t ybn_alloc(void) {
    ybn_t bn = (ybn_t)malloc(sizeof(struct ybn));
    if (bn == NULL)
        return NULL;
...
static uint32_t Step_D3(uint16_t *uj, uint16_t *v, int n) {
    uint32_t hat = (((uint32_t)uj[n])<<16) + uj[n-1];
...
int ybn_div(ybn_t quotient, ybn_t remainder, ybn_t numerator, ybn_t denominator) {
    ...
    memset(q, 0, (m+1)*sizeof(uint16_t));
...
ybn_t ybn_dup(ybn_t bn) {
    ybn_t res = (ybn_t)malloc(sizeof(struct ybn));
    res->ybn_sign = bn->ybn_sign;
    res->ybn_size = res->ybn_len = bn->ybn_len;
    res->ybn_data = (uint16_t *)malloc(bn->ybn_len * sizeof(uint16_t));
    memcpy(res->ybn_data, bn->ybn_data, bn->ybn_len * sizeof(uint16_t));
    return res;
}

void ybn_merge(ybn_t into, ybn_t from) {
    into->ybn_sign = from->ybn_sign;
    into->ybn_len = from->ybn_len;
    into->ybn_size = from->ybn_size;
    free(into->ybn_data);
    into->ybn_data = from->ybn_data;
    free(from);
}
...
static int ybn_submagnitude(ybn_t result, ybn_t a, ybn_t b) {
    ...
    uint32_t carry = (uint32_t)-borrow;
...
int ybn_sqr(ybn_t result, ybn_t a) {
    ...
    for (int i = 0; i < alen; i++) {
        uint64_t carry = rd[i*2] + (uint64_t)ad[i] * ad[i];  // a^2
...
int ybn_modexp(ybn_t result, ybn_t base, ybn_t exp, ybn_t modulus) {
    ...
    for (int i = ybn_reallen(exp); i-- > 0; ) {
        for (uint16_t m = 0x8000; m; m>>=1) {
            ...
            ybn_div(quot, result2, tmp, modulus);

            if(condition){
                ybn_resize(result, result2->ybn_size);
                result->ybn_len = result2->ybn_len;
                memcpy(result->ybn_data, result2->ybn_data, result2->ybn_len * sizeof(uint16_t));
            }
        }
    }
    ybn_free(tmp);
...
int copyYBN(ybn_t destYBN, ybn_t sourceYBN) {
    destYBN->ybn_sign = sourceYBN->ybn_sign;
    destYBN->ybn_len = sourceYBN->ybn_len;
    if (ybn_resize(destYBN, sourceYBN->ybn_size) == -1) return -1;
    memcpy(destYBN->ybn_data, sourceYBN->ybn_data, sizeof(uint16_t) * sourceYBN->ybn_size);
    return 0;
}
