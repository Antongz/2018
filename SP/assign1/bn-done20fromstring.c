...
bool isValidAddition(uint16_t x, uint16_t y, uint16_t carry) {
...
int copyBN(bn_t destBN, bn_t copyBn) {
    ...
    bn_resize(destBN, copyBn->bn_size);
...
int bn_add(bn_t result, bn_t a, bn_t b) {
    int i = 0;
    uint16_t d_a = 0x0000, d_b = 0x0000, carry = 0x0000;

    int maxLen = MAX(a->bn_len, b->bn_len);

    bn_resize(result, maxLen+1);
    ...
    for (i=0; i<maxLen; i++) {
        ...
        if (isValidAddition(d_a, d_b, carry)) {
            result->bn_data[i] = d_a + d_b + carry;
            carry = 0x0000;
        }

        // overflow happens
        else {
            result->bn_data[i] = (uint16_t)(d_a - (MAXUINT16 - d_b) + carry - 1);
            carry = 0x0001;
            if (i == maxLen-1) {
                result->bn_len++;
...
int bn_sub(bn_t result, bn_t a, bn_t b) {
    ...
    // when a->bn_len >= b->bn_len
        ...
        if ((a->bn_len == b->bn_len) && (a->bn_data[a->bn_len-1]<b->bn_data[b->bn_len-1])) {
            bn_free(result);
            result = bn_alloc();
            return 0;
        }
...
int bn_mul(bn_t result, bn_t a, bn_t b) {
    ...
    bn_resize(result, a->bn_len + b->bn_len + 1);
...
int bn_fromString(bn_t bn, const char *s) {
    ...
    if (len < 6) {
        ...
        bn_t b = bn_alloc();
        bn_t base2 = bn_alloc();
        bn_t baseUINT16 = bn_alloc();

        bn_fromString(a, "2");
        bn_fromString(base2, "2");
        bn_fromString(baseUINT16, "65536");
        bn_fromString(b, "1");
        ...
        while (itr >= 0) {
            ...
            bn_sub(tempBN, tempBN, c);
            memset (buf, 0, 10000);
            bn_toString(tempBN, buf, sizeof (buf));

            if ((strcmp(s, buf) <= 0) || (strlen(s) < strlen(buf))) {
                printf("\nSUB: tempBN: %s\n", buf);
                copyBN(a, tempBN);
            }
