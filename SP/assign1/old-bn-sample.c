...
int bn_sub(bn_t result, bn_t a, bn_t b) {
    int i = 0, j = 0, realSize = 0 , maxLen = MAX(a->bn_len, b->bn_len);
    ...
    bn_t newA = bn_alloc();
    bn_resize(newA, a->bn_len);
    copyBN(newA, a);

    printf("a(size): %d, a(length): %d; b(size): %d, b(length): %d\n", a->bn_size, a->bn_len, b->bn_size, b->bn_len);

    if (newA->bn_len < b->bn_len) {
        bn_free(result);
        result = bn_alloc();
        result->bn_len = 1;
        return 3;
    }
//    else if ((newA->bn_len == b->bn_len) && (newA->bn_data[maxLen-1] < b->bn_data[maxLen-1])) {
//        bn_free(result);
//        result = bn_alloc();
//        result->bn_len = 1;
//        return 2;
//    }
    ...
    // when a->bn_len >= b->bn_len
        ...
        for (i=0; i<maxLen; i++) {
            ...
            if (!isValidSubstraction(d_a, d_b)){
                j = i;
                while (j<maxLen) {
                    if (newA->bn_data[j] > b->bn_data[j]) {
                        newA->bn_data[j] = (uint16_t) (newA->bn_data[j] - 1);
                        carry = MAXUINT16;
                ...
                newR->bn_data[i] = (uint16_t) (newA->bn_data[i] + ((MAXUINT16) - b->bn_data[i]) + 1);

            }

            else {
                newR->bn_data[i] = (uint16_t) ((d_a - d_b) + carry);
            }
        }

//        // Check whether a - b = 0
//        for (i=newA->bn_len; i>=0; i--) {
//            if (newR->bn_data[i] != 0) {
//                realSize = i;
//                newR->bn_len = i + 1;
//                break;
