...
int main() {
    ...
    while (status != -1) {
        ...
        if (isspace(s) || (s == '\n') || (s == EOF)) {
            ...
            else if (strcmp(c, "sqr") == 0) {
                if ((operand1 = pop(&root)) == NULL) { status = -1; break; }
                if ((status = bn_sqr(operand1, operand1)) == -1) { status = -1; break; }
                push(&root, operand1);
            }
            else if (strcmp(c, "/") == 0) {
                bn_t quo = bn_alloc();
                if (quo == NULL) { status = -1; break; }
                bn_t rem = bn_alloc();
                if (rem == NULL) { status = -1; break; }
                if ((operand2 = pop(&root)) == NULL) { status = -1; break; }
                if ((operand1 = pop(&root)) == NULL) { status = -1; break; }
                if ((status = bn_div(quo, rem, operand1, operand2)) == -1) { status = -1; break; }
                // push back quot
                push(&root, quo);
                // push back rem
                push(&root, rem);
            ...
            else if (strcmp(c, "modexp") == 0) {
                bn_t res = bn_alloc();
                if (res == NULL) { status = -1; break; }
                if ((operand3 = pop(&root)) == NULL) { status = -1; break; }
                if ((operand2 = pop(&root)) == NULL) { status = -1; break; }
                if ((operand1 = pop(&root)) == NULL) { status = -1; break; }
                if ((status = bn_modexp(res, operand1, operand2, operand3)) == -1) { status = -1; break; }
                // push back quot
                push(&root, res);
                bn_free(operand1);
                bn_free(operand2);
