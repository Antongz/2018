...
public class NewsAggregator {
    ...
    static ArrayList<Integer> NewsCreatorID = new ArrayList<>();
    ...
    /* This part is for responsing requests from NewsCreator(NC) */
    public void response_NC_requests(int id) throws SecurityException {
        int size = NewsCreatorID.size();
        File registerFile;
    ...
    // If the id is already registered, it will return the index in NewsCreatorID
    // Otherwise, returns -1.
    public int check_if_NC_registrated(int id) {
        int size = NewsCreatorID.size();

        for (int i = 0; i < size; i++) {
            if (NewsCreatorID.get(i) == id) {
                return i;
            }
        }
        return -1;
    ...
    public void write_logs() {
        ...
        try {
            ...
            System.out.println("Finish our logfile");
...
// Thread class that receives NCID news
...
class NASender extends NewsAggregator {
    ...
    /*This part is for responsing requests from Clients(C)*/
    public void sendNews_to_Client(int C_ID) throws IOException {
        log_buffer += "NA -REQUEST- from Client_" + Integer.toString(C_ID)
        + "\n";

        System.out.println("NA -REQUEST- from Client_" + Integer.toString(C_ID));
        news_NAS = news;
