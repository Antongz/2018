...
/* News Creator(NC):
* 1.) Created with an ID and a news creation rate(NCR) both command line
*     parameters.
* 2.) Registers its ID to the NA
* 3.) Based on the rate, the NC creates a random piece of news and sends it
*     to the NA as plain text, in a file
*/
...
public class NewsCreator {
    ...
    public static void main(String[] args) {
        ...
        NewsCreator nc = new NewsCreator(Integer.parseInt(args[0]),
        Integer.parseInt(args[1]));

        File approve = new File("NA/Approve_NC" + Integer.toString(nc.get_NC_id()));

        // If the current NC doesn't get response for the registration, it will
    ...
    // when a news creator NCID registers with NA, it creates a file
    ...
    // Once the NC creates a news file, it calls RandomArticleProducer to
    ...
    public void create_news() {
        ...
        RandomArticleProducer.genFile(path);
    }
