...
public class MapReaderWriter implements MapIo
{
    ...
    public void read (Reader r, Map m) throws IOException, MapFormatException {
        if (r==null)    throw new IOException("Reader cannot be null");
        if (m==null)    throw new IOException("Map cannot be null");
        ...
        while (line != null) {
            ...
            if ((result[0].equals("place")) && (result.length == 4)) {
                ...
                Place p = m.findPlace(plcname);
                if (p != null)  m.setEndPlace(p);
                else    throw new MapFormatException(linenr,"End place \""+ plcname + "\" is not in the map");
            } else {
