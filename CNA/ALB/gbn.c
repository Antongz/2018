...
void A_output(struct msg message)
{
    ...
    if ( windowcount < WINDOWSIZE) {
        ...
        A_nextseqnum = (A_nextseqnum + 1) % WINDOWSIZE;  /* we only have seqnum 0 and 1 */
    }
...
void A_input(struct pkt packet)
{
    ...
    if (!IsCorrupted(packet)) {
        ...
        if (packet.acknum==tempACK) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
            /* packet is a new ACK */
            if (TRACE > 0)
