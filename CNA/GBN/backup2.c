...
static struct pkt buffer[buffersize];  /* array for storing packets waiting for ACK */
...
void A_output(struct msg message)
{
    ...
    if ( A_nextseqnum < base+WINDOWSIZE) {
        ...
        windowlast = (windowlast + 1) % buffersize;
...
void A_input(struct pkt packet)
{
    ...
    if (!IsCorrupted(packet)) {
        ...
        if (packet.acknum>=base) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
            ...
            windowfirst = ((windowfirst + (packet.acknum - base) + 1)) % buffersize;

            if(( windowlast + 1 ) % buffersize == windowfirst){
                windowfirst = 0;
                windowlast = -1;
                windowcount = 0;
            }else{
            ...
            else if(base==A_nextseqnum){
                stoptimer(A);
            }

            tempHead = windowfirst;
            i = A_nextseqnum;
            while(i<base+WINDOWSIZE){
                if(i<=windowlast){
                    windowcount++;

                    /* send out packet */
                    if (TRACE > 0)
                        printf("Sending packet %d to layer 3\n", buffer[tempHead].seqnum);
                    tolayer3 (A, buffer[tempHead]);
...
void A_timerinterrupt(void)
{
    ...
    for(i=0;i<windowcount;i++){
        ...
        packets_resent++;
        temp_head = (temp_head + 1) % buffersize;
    }
